...
predictModel.test = predict(model, newdata=test, type="response") 
predictModel.train = predict(model, newdata=train, type="response")

#Split up the model into spam and 
#not spam
probability.test <- ifelse(predictModel.test > 0.5, "1", "0")
probability.train <- ifelse(predictModel.train > 0.5, "1", "0")

confMatrix.test <- table(probability.test, test[,"Spam"]) #Confusionmatrix from the model
#Diagonal of the misclassfication rate by dividing the 
#diagonal from the confusionmatricx with the whole confusionmatrix
modelDiag.test <- diag(confMatrix.test) 
missClMa1.test = 1-(sum(modelDiag.test)/sum(confMatrix.test))

confMatrix.train <- table(probability.train, train[,"Spam"]) #Confusionmatrix from the model
#Diagonal of the misclassfication rate by dividing the 
#diagonal from the confusionmatricx with the whole confusionmatrix
modelDiag.train <- diag(confMatrix.train) 
missClMa1.train = 1-(sum(modelDiag.train)/sum(confMatrix.train)) 

#Prints results
print("Confusion matrix test:")
print(confMatrix.test)
print(paste("missclassification test:",missClMa1.test))

#Prints results
print("Confusion matrix train:")
print(confMatrix.train)
print(paste("missclassification test:",missClMa1.train))
```
...
probability.test <- ifelse(predictModel.test > 0.8, "1", "0")
probability.train <- ifelse(predictModel.train > 0.8, "1", "0")

confMatrix2.test <- table(probability.test, test[,"Spam"]) 
modelDiag2.test <- diag(confMatrix2.test)
missClMa2.test = 1-(sum(modelDiag2.test)/sum(confMatrix2.test))

confMatrix2.train <- table(probability.train, train[,"Spam"]) 
modelDiag2.train <- diag(confMatrix2.train)
missClMa2.train = 1-(sum(modelDiag2.train)/sum(confMatrix2.train))


print("Confusion matrix test:")
print(confMatrix2.test)
print(paste("missclassification test:",missClMa2.test))

print("Confusion matrix train:")
print(confMatrix2.train)
print(paste("missclassification test:",missClMa2.train))
```
